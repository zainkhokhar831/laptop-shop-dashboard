...
class Email {
	...
	public static function from_data( $data ) { // phpcs:ignore Generic.Metrics.CyclomaticComplexity.TooHigh
		...
		if ( is_null( $data ) ) {
			throw new Exception( esc_html__( 'Record not found in DB', 'easy-wp-smtp' ) );
		}
		...
		if (
			...
			throw new Exception( esc_html__( 'Invalid record format', 'easy-wp-smtp' ) );
		}
		...
		if ( ! is_null( $data->data ) && ! WP::is_json( $data->data ) ) {
			throw new Exception(
				...
				/* translators: %1$s - JSON error message. */
					esc_html__( 'Data JSON decoding error: %1$s', 'easy-wp-smtp' ),
		...
		if ( ! empty( $data ) ) {
			...
			if ( $data === false ) {
				throw new Exception(
					sprintf(
					/* translators: %1$s - JSON error message. */
						esc_html__( 'Data JSON encoding error: %1$s', 'easy-wp-smtp' ),
						esc_html( json_last_error_msg() )
					)
				);
			}
		...
		if ( $result === false ) {
			throw new Exception(
				...
				/* translators: %1$s - Database error message. */
					esc_html__( 'Insert/update SQL query error: %1$s', 'easy-wp-smtp' ),
